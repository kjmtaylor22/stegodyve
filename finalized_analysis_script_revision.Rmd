...
```{r 00_gradient_Figure1Av2, fig.width=16.5, fig.height=4.5}
...
egg::ggarrange(
  ggplot(nam, aes(x=-latitude, y=value, color=variable, fill=variable, alpha=value)) + theme_bw() + 
    ...
    geom_label(aes(y=lab_y, label=label), color="#042245", fill="white", alpha=1, fontface="bold", size=4, 
...
#                              label=list(bquote("y="~.(b0)~"+"~.(b1)~"x, "~R^2==.(R2))))) +
    ...
    coord_cartesian(ylim=c(18,21)),
  ggplot(saf, aes(x=longitude, y=value, color=variable, fill=variable, alpha=value)) + theme_bw() + 
...
```{r 00_gradient_Figure1Av1, fig.width=16.5, fig.height=4.5}
...
```{r 01_SampleComposition_fig2, fig.width=9, fig.height=4.5, echo=F, eval=F}
egg::ggarrange(
...
```{r 01_SampleComposition_fig2_alt, fig.width=9, fig.height=4.5}
egg::ggarrange(
...
partitioning <- function(sa, gm){
  bysamp <- function(sampletype, col){
    ...
    plotVenn <- draw.pairwise.venn(fracts$sets[1], fracts$sets[2],fracts$sets[3], 
                                   ...
                                   cat.fontfamily="sans", cat.cex=c(0.7,0.7),
...
```{r 02_Processes_DistDecay, fig.width=5.5, fig.height=9}
...
#### Supplementary
```{r 02_Processes_RegressionEnv, fig.width=11, fig.height=9}
...
g5 <- ggplot(sample_dist, aes(envgrad, cross)) + 
  ...
  ggpmisc::stat_poly_eq(formula=y ~ x, parse=T, label.x = "left",
                        label.y="bottom",vjust=-0.5, vstep=-0.3,
                        aes(linetype=transect.wb,
                            label=paste("bold(\"",
                                        c("Within","Between"
                                          )[stat(linetype)],
  ...
  guides(shape=guide_legend(title=NULL, direction="horizontal", byrow = F,
...
vennplot <- function(venn){
  vp <- ggdraw() + draw_plot(as_grob(venn)) +
    ...
    draw_plot_label(paste("R^2 ==", sum(venn$fracts$fracts[-4])),0.15,0.3, 
                  fontface="plain", size=9, parse=T)
  return(vp)
}
...
vp.img <- grid::viewport(x=unit(0.55,"npc"), y=unit(0.75,"npc"), 
                         width=unit(0.4, "npc"), height=unit(0.55,"npc"), just = "right")
...
```{r 02_Processes_FigureS4, fig.width=13, fig.height=9}
...
```{r 03_CrossCompar_setup, fig.width=4, fig.height=7}
...
```{r 04_SourceTracker_Figure4, fig.height=4.5, fig.width=9.5}
