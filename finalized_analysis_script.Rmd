...
#### Figure 4
```{r 02_SourceTracker_plots1, fig.width=4.5, fig.height=7, cache=T}
...
ggplot(g0) + 
  ...
  theme(strip.text=element_text(family="mono", face="bold", size=18, angle=-90),
        strip.background = element_blank()) +
  labs(x="Mean CV of Gibbs draws", y="n ASVs") + coord_cartesian(expand=F) +
  geom_density(aes(Soil, after_stat(count)), color=colorblind[3], fill=colorblind[3], alpha=0.3, linewidth=1.8) +
  geom_density(aes(Retreat, after_stat(count)), color=colorblind[2], fill=colorblind[2], alpha=0.3, linewidth=1.8) +
  geom_density(aes(Unknown, after_stat(count)), color=colorblind[7], fill=colorblind[7], alpha=0.3, linewidth=1.8) 
```
...
```{r 02_SourceTracker_plots2, eval=T, echo=T}
ggplot(meantop) + 
  ...
  scale_fill_manual(values=colorblind[c(3,6,2,7)]) +
...
```{r 02_SourceTracker_plots3, eval=F, echo=T, fig.width=5, fig.height=5}
...
```{r 02_SourceTracker_plots4, cache=F}
## summarizes previous plot in a pie chart
ggplot(identifysource) + 
  facet_wrap(vars(set), strip.position = "top", nrow=1) +
  geom_point(aes(Retreat, Retreat.1), color=colorblind[2], shape=1) +
  geom_point(aes(Soil, Soil.1), color=colorblind[3], shape=1) +
  geom_point(aes(Unknown, Unknown.1), color=colorblind[7], shape=1, alpha=0.5) +
  ...
  xlab("Mean assignment proportion") + ylab("CV") + ggthemes::theme_few()

## summarizes previous plot in a pie chart
ggplot(verifysource) + 
  facet_grid(set ~ g) +
  geom_point(aes(prop, CV), shape=1) +
  stat_smooth(aes(prop, CV), method="lm", formula=y~log(x)) +
...
for (i in unique(sample$set)){
  if (i=="Bacteria") {main=expression("Average source assignment")}else{main=""}
  ...
  if (i=="Bacteria") {main=expression("ASV source assignment")}else{main=""}
  TernaryPlot(point = "up", atip = "", btip = "", ctip = "", main=eval(main),
              alab = "Unknown", blab = "Soil", clab = "Retreat")
...
#     scale_color_manual(values=colorblind[c(3,6,2,7)]) +
...
```{r 02_SourceTracker_FigureS4c, fig.height=4, fig.width=6, fig.show='hold'}
...
# ggplot(try1) +   ggthemes::theme_few() + 
#   facet_grid(source ~ sample_type, scales="free_y") +
#   geom_point(aes(site_proportion, assignment), alpha=0.6, pch=21) +
#   stat_smooth(aes(site_proportion, assignment), method="lm") +
...
try2 <- bact$CoefVar$CV %>% #.[,-1] %>%
  #group_by(tag) %>% summarize_all(mean, na.rm=T) %>%
...
# ggplot(try2) +   ggthemes::theme_few() + 
#   facet_grid(source ~ sample_type, scales="free_y") +
#   geom_point(aes(site_proportion, assignment), alpha=0.6, pch=21) +
#   stat_smooth(aes(site_proportion, assignment), method="lm") +
#   scale_x_continuous(expand=c(0.1,0)) +
...
```{r, 02_SourceTracker_FigureS5e, fig.height=5, fig.width=8, fig.show='hold'}
egg::ggarrange(
  ggplot(g0) + 
    facet_wrap(vars(set), scales = "free_y", strip.position = "left", ncol=1) +
    ggthemes::theme_clean(base_size = 18) + 
  ...
  ggplot(try6) + ggthemes::theme_few(base_size = 15, base_family = "sans") + 
    ...
    #scale_x_continuous(expand=c(0.15,0)) + #, limits=c(0,1), n.breaks=3) +
    #scale_x_log10() +
    scale_y_continuous(position="right") +
    scale_color_manual(values=colorblind[c(2,3,7)]) +
...
egg::ggarrange(
  ...
  ggplot(try6) + ggthemes::theme_few(base_size = 15, base_family = "sans") + 
    ...
    #stat_smooth(aes(log(mean_CV), site_proportion, color=source), method="lm") +
...
```{r 02_SourceTracker_plots5, fig.height=5, fig.width=8}
egg::ggarrange(
...
##### Bacteria
```{r 02_SourceTracker_FigureS5a, fig.height=11, fig.width=14}
...
core.listB <- set_listB$tag %>% unlist() %>% unique() %>% .[1:30] %>% na.omit()

ggB1 <- dendro.heatmap(commB[row.names(tmpmetaB),], taxB, tmpmetaB, #unifracB$unifracs[,,"d_UW"],
                        as.matrix(vegan::vegdist(commB[row.names(tmpmetaB),], binary = T)),
                       "data/bacteria/tree.nwk", "WebType", "label", core.listB, "tag.name")

#ggpubr::annotate_figure(ggB1$whole, top = ggpubr::text_grob("Bacteria: Venn Top 30 (median) - Dendrogram (binary Bray)",
#                                               face = "bold", size = 18))
...
ggB2 <- ggB2$heat.raster+
  ...
  theme(panel.spacing = unit(1,"mm"),
        ...
        plot.margin = margin(1,11,1,1,"cm")) +
...
ggB3 <- ggpubr::annotate_figure(ggB2, top = ggpubr::text_grob("Bacteria: Top 30 (median relative abundance) + SourceTracker assignments",
                                               face = "bold", size = 18))
...
ggB3 + 
  patchwork::inset_element(pie_grob, left = 0.84, bottom = 0.83, right = 0.98, top = 1) +
  patchwork::inset_element(venn_grob, left = 0.02, bottom = 0.83, right = 0.16, top = 1)

# st_listB <- bact$TopTaxa %>% .[order(.$gibbs.mean, decreasing=T),] %>% .$tag %>% unique()
# 
# ggB3 <- dendro.heatmap(commB[row.names(tmpmetaB),], taxB, tmpmetaB, #unifracB$unifracs[,,"d_UW"],
#                         as.matrix(vegan::vegdist(commB[row.names(tmpmetaB),], binary = T)),
#                        "data/bacteria/tree.nwk", "WebType", "label", st_listB, "tag.name")
# 
# ggB3$heat.raster@data$name <- as.character(ggB3$heat.raster@data$name)
# ggB3$heat.raster@data <- left_join(ggB3$heat.raster@data, set_listB) %>%
#   left_join(tmpmetaB[,c("newlab","WebType")], by=c("id"="newlab")) %>% 
#   left_join(taxB[,c("tag","taxonomy")]) %>%
#   left_join(bact$CoefVar$CV[,c(1,5)], by=c("tag"="tag")) %>%
#   .[order(.$sec.list, .$taxonomy, .$abund),] %>%
#   mutate(source = factor(g.cv, labels=c("3","2","7")))
# ggB3$heat.raster@data$name <- factor(ggB3$heat.raster@data$name, unique(ggB3$heat.raster@data$name))
# ggB3$heat.raster@data$source <- as.numeric(as.character(ggB3$heat.raster@data$source))
# ggB3$heat.raster@data$source[is.na(ggB3$heat.raster@data$source)] <- 8
# 
# ggB3 <- ggB3$heat.raster+
#   facet_grid(sec.list~WebType,space="free",scales="free") +
#   theme(panel.spacing = unit(1,"mm"),
#         strip.text.y = element_text(angle=0, size=14, face="bold"),
#         strip.text.x = element_text(angle=0, size=14, face="bold"),
#         axis.text.y = element_text(color=colorblind[unique(ggB3$heat.raster@data[,c(3,5,9,10)])$source])) 
# 
# ggpubr::annotate_figure(ggB3, top = ggpubr::text_grob("Bacteria: SourceTracker Top 10 (Gibbs mean) - with assignments",
#                                                face = "bold", size = 18))
```

##### Fungi
```{r 02_SourceTracker_FigureS5b, fig.height=11, fig.width=14}
...
core.listF <- set_listF$tag %>% unlist() %>% unique() %>% .[1:30] %>% na.omit()
...
ggF3 <- ggpubr::annotate_figure(ggF2, top = ggpubr::text_grob("Fungi: Top 30 (median relative abundance) + SourceTracker assignments",
                                               face = "bold", size = 18))
...
#### Figure S6
```{r 03_CrossCompar_FigureS6, fig.width=14, fig.height=8}
...
g2 <- ggplot(df, aes(dist*111.11, cross, linetype=transect.wb)) + 
  ...
  ggpmisc::stat_poly_eq(formula=y ~ x, parse=T, label.x = "left", label.y="bottom", 
                        vjust= 0, size=2.5, vstep=0.7, hstep=0.9, 
...
g4 <- ggplot(df, aes(dist*111111+1, cross))+#, linetype=transect.wb)) + 
  facet_grid(nestedness ~ set) +
  geom_point(aes(shape=compare, color=nestedness), show.legend = c(color=F)) +
  stat_smooth(method="lm", formula='y~I(-2*x+1)', color="black", fullrange=T)+
  geom_text(data=ztab, aes(x=3, y=0, 
                           label=paste0("z=",z,"\np=",`p-val`)),
  ...
  theme(plot.margin=unit(c(1.5,0.5,0,0.5), "lines"),
        ...
        strip.text.y = element_text(hjust=0.5, face="bold", family="mono", size=14),
        strip.background = element_blank(), strip.clip="off",) +
  xlab("Distance between colonies (m)") + ylab("Compositional similarity")
```
